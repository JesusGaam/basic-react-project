...
export const getLeadType = (utms) =>
  utms?.gclid || utms?.fbclid ? "paid" : "organic";
...
export const isMediaScreen = (breakpoint = 420, majorToBreakpoint = false)  =>{
  if (!majorToBreakpoint) {
      breakpoint += 1;
  }
  return window.matchMedia("only screen and ("+ (majorToBreakpoint? "max": "min") + "-width: " + breakpoint + "px)").matches;
}

export const insertScript = (src, onload) =>{
